...
game.mechanic.generateItem = function(max_object_value) {
    var equip_value = Number.prototype.random(0, max_object_value);
    ...
    if (equip_value >= 75) {
        ...
        if (equip_type !== "artefact") {
            ...
            item = new game.ItemObject(name, icon_image, equip_type, rarity, attributes, tooltip);
        }
    }
    return item;
};
...
game.mechanic.drop = function(x, y, container_value, drop_table) {
    ...
    if (chance_equip <= drop_table.equip) {
        var item = game.mechanic.generateItem(max_object_value);
        if (item !== null) {
