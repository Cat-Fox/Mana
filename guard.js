game.Guard = me.ObjectEntity.extend({
    sparks: null,
    time: null,
    hp: 100,
    target: null,
    ...
    init: function(x, y, settings) {
        ...
        this.renderable.addAnimation("idle", [0, 1], 35);
        this.renderable.setCurrentAnimation("idle");
        this.type = "npc";
        this.updateColRect(5, 17, 2, 18);

        this.shadow = new game.Shadow(this.pos.x + 6, this.pos.y + 8);
        ...
        if (res.length >= 1) {
            for (var i = 0; i< res.length; i++) {
                if (res[i].obj.type === "human_target") {
                    ...
                    if (this.target === null) {
                        this.target = new game.Target(this.pos.x + 6, this.pos.y + 8, "green");
                ...
                if (res[i].obj.type === "human_use") {
                    if (this.use === null) {
                        ...
                        this.use = new game.Message(this.pos.x - 25, this.pos.y - 40, text);
    ...
    createSparks: function() {
        if (this.sparks === null) {
            this.sparks = new game.Sparks(this.pos.x, this.pos.y);
